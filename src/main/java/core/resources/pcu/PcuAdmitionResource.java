...
public class PcuAdmitionResource {
	...
	// http://localhost:8080/HIS_API/rest/PcuAdmition/SelectAllPatient
	@GET
	@Path("/SelectAllPatient")
	@Produces(MediaType.APPLICATION_JSON)
	public String SelectAllPatient() {
		try {
			List<OutPatient> pcuAdmitionResult = pcuAdmitionDBDriver
					.SelectAllPatient();
	...
	public String Save(JSONObject newObject) {
		...
		try {
			userObj = new UserDBDriver().getUserDetailsByUserID(
					newObject.getInt("created_by")).get(0);
			OutPatient outPatientObj = new PatientDBDriver()
					.getPatientDetails(newObject.getInt("patient_id"));
			PcuAdmition newPcuAdmition = new PcuAdmition();
			newPcuAdmition.setPatientId(outPatientObj);
			newPcuAdmition.setAdmitionDate(new SimpleDateFormat("yyyy-MM-dd")
					.parse(newObject.getString("admition_date")));
	...
	public String Update(JSONObject updateObject) {
		...
		try {
			userObj = new UserDBDriver().getUserDetailsByUserID(
					updateObject.getInt("modified_by")).get(0);
			String status = "";
			if (pcuAdmitionDBDriver.Update(updateObject.getInt("admition_id"),
					updateObject.getString("status"), new SimpleDateFormat(
							"yyyy-MM-dd").parse(updateObject
							.getString("admition_date")), userObj,
					new SimpleDateFormat("yyyy-MM-dd HH:mm:ss")
							.parse(updateObject.getString("modified_time")))) {
