...
//		String patientHin=GenerateHin.getcompleteHin();
		...
		try {
			...
			patientDBDriver.insertPatient(patient,userid);
	...
	public String PatientDetails(@PathParam("patientId")int patientId)
	{
		...
		return jsonSerializer.exclude(
				"patientLastUpDateUser.specialPermissions","patientLastUpDateUser.userRoles",
				"patientCreateUser.specialPermissions", "patientCreateUser.userRoles"
				).include("allergies","visits","exams","attachments","records","laborders","answerSets","answerSets.questionnaireID").serialize(patient);
	}
	...
	public String changePatientDetails(JSONObject pJson)
	{
		try {
			...
			patientDBDriver.updatePatient(patientid,patient,userid);
			return "True";	
	...
	public String getPatientList()
	{
		...
		return  serializer.exclude("*.class").transform(new DateTransformer("yyyy-MM-dd"),"patientCreateDate","patientLastUpdate").serialize(patientList);
	}
