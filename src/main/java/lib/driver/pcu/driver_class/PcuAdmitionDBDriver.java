...
public class PcuAdmitionDBDriver {
	...
	public List<OutPatient> SelectAllPatient() {
		Transaction tx = null;
		try {
			tx = session.beginTransaction();
			Query query = session.createQuery("from OutPatient as p");
	...
	public boolean Update(int admition_id, String status,Date admition_date, AdminUser modified_by, Date modified_time) {
		Transaction tx = null;

		try {
			PcuAdmition updatePcuAdmition = (PcuAdmition) session.load(PcuAdmition.class, admition_id);
			tx = session.beginTransaction();
			updatePcuAdmition.setAdmitionDate(admition_date);
