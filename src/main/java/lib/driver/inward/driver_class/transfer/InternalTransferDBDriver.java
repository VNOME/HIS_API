...
public class InternalTransferDBDriver {
	...
	*/
	...
	public boolean updateInternalTransferDetails(int id){
	Transaction tx=null;
	...
	try{
		...
		/*Query q = session.createQuery("from InternalTransfer  where transferId = :transferId ");
		   q.setParameter("transferId", id);
		   InternalTransfer trans = (InternalTransfer)q.list().get(0);
		   */
		/*InternalTransfer trans = (InternalTransfer) session.get(InternalTransfer.class,1);
		  trans.setRead(1);
		  */ 
		
		String hql = "UPDATE InternalTransfer set read = 1 "  + 
