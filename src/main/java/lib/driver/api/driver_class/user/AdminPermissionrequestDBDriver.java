...
public class AdminPermissionrequestDBDriver extends DBDriverBase<AdminPermissionrequest>  {
	...
	public List<AdminPermissionrequest> getUnApprovedPermissionRequests(){
		Transaction tx = null;
		
		try{
			tx = session.beginTransaction();
			Query query =  session.createQuery("select pr from AdminPermissionrequest as pr where pr.isActive = true and pr.adminUserByApprover = null");		
			List<AdminPermissionrequest> permissionRequestList =CastList.castList(AdminPermissionrequest.class, query.list()); 
			tx.commit();
			return permissionRequestList;
	...
	public List<AdminPermissionrequest> getApprovedPermissionRequest(int requester){
		Transaction tx = null;
		
		try{
			tx = session.beginTransaction();
			Query query =  session.createQuery("select pr from AdminPermissionrequest as pr where  pr.adminUserByRequester.userId =:requester and pr.adminUserByApprover is not null and pr.isApprove = true");		
