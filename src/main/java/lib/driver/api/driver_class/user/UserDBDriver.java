...
//	public boolean updateUserDetails(AdminUser usrObj,int userRoleID){
	...
	public boolean updateUserDetails(AdminUser usrObj){
//		public boolean updateUserDetails(AdminUser usrObj,int userRoleID){
			Transaction tx=null;
...
//				HrEmployee emp=(HrEmployee)session.get(HrEmployee.class, userEmployeeID);
				...
				user.setUserName(usrObj.getUserName());
				user.setPassword(usrObj.getPassword());
				user.setAdminUserroles(usrObj.getAdminUserroles());
				user.setHrEmployee(usrObj.getHrEmployee());
	...
	public List<AdminUser> getUserDetailsByUserRole(String roleName)
	{
		Transaction tx = null;
		...
		{
			tx = session.beginTransaction();
			Query query = session.createQuery("select u from AdminUser as u where u.userRoles.userRoleName=:role");
	...
	public List<AdminUser> getUserRolePermissionById(int userId,int roleId){
		Transaction tx = null;
		
		try{
			tx = session.beginTransaction();
			Query query =  session.createQuery("select u from AdminUser as u where u.isActive = true AND u.adminUserroles.roleId=:roleId AND u.userId=:userId");
			query.setParameter("roleId", roleId);
			query.setParameter("userId", userId);
			List<AdminUser> userRoleList =CastList.castList(AdminUser.class, query.list()); 
			tx.commit();
			return userRoleList;
