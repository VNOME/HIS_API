...
public class HrDepartmentDBDriver {
	...
	public Boolean InsertNewDepartment(String deptName, int deptHead) {
		...
		try {
			...
			HrEmployee empO = (HrEmployee) session.get(HrEmployee.class, // department  head ge emp object gannawa
					deptHead);
			...
			if (tx != null && tx.isActive()) {
				try {
					...
					System.err.println("Error rolling back transaction"); // handling exxxxxxxxxxxx
				}
				throw ex;
			}
			return false;
